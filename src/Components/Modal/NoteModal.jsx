            ...
            <dialog ref={dialog} className={s.favDialog}>
                ...
                <div className={s.favDialogClass}>
                    <form method="dialog" onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <div><textarea {...register("description",{required:"Поле обязательно"})} placeholder="Описание заметки*" /></div>
                        ...
                        <div>{idExisted.length === 0 ? <button type="submit" className={s.addButton}>Добавить</button> : <button type="submit" className={s.changeButton}>Изменить</button>}</div>
                    </form>
                </div>
            </dialog>
        </>
